...
> **Windows:** `app/main.py` and `tests/conftest.py` switch asyncio to the
> Selector event loop. Without it, every async database call fails with
> *"Psycopg cannot use the 'ProactorEventLoop'"*. It is a no-op elsewhere.
