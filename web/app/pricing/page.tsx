...
export const metadata: Metadata = {
  ...
  description: "14-day founder trial, then a one-time unlock. Investors are thesis-gated, not paid.",
};

export default function PricingPage() {
  return (
    <LandingShell eyebrow="PRICING" title="One audit desk. One payment.">
      <p>
        Founders get a {TRIAL_DAYS}-day trial with the full desk: intake, audit, readiness tasks,
        mentor, and publish. After that, a one-time {UNLOCK_PRICE.label} unlock restores access. It
        is not a subscription.
      </p>
      <div className="grid gap-4 md:grid-cols-2">
        <section className="border border-line bg-surface p-6">
          <p className="text-[11px] font-extrabold tracking-[0.16em] text-brass">FOUNDERS</p>
          <h2 className="mt-3 font-display text-2xl text-cream">{UNLOCK_PRICE.label} once</h2>
          <p className="mt-2">
            {TRIAL_DAYS} days free, then the unlock. Includes dealflow listing when you publish,
            the AI mentor, and catalogue programmes sold separately.
          </p>
          <Link
      ...
      <p>
        ...
        Stripe Checkout once SACI attaches a Price. Already unlocked?{" "}
        <Link href="/founder/paywall" className="text-brass hover:text-cream">
