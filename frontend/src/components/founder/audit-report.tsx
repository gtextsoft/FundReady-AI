...
function verdictColour(verdict: Verdict): string {
  ...
  if (verdict.level === 'fundable' || verdict.level === 'saleable') return C.grn;
...
function VerdictPanel({ verdict, title }: { verdict: Verdict; title: string }) {
  ...
  return (
    <View className="flex-1 rounded-[12px] border border-line bg-surface-1 p-[14px]">
      ...
      <View className="mb-[6px] mt-[10px] flex-row items-baseline gap-[7px]">
        {unknown ? (
          ...
          <>
            ...
            <Txt className="text-[12px] text-ink-faint">/ 100</Txt>
          </>
        )}
      </View>
      ...
      <Txt className="mt-[9px] text-[12px] text-ink-muted" style={{ lineHeight: 18 }}>
        {verdict.rationale}
      </Txt>

      {/* What the audit could not see is part of the verdict, not a footnote. */}
      {verdict.unevidencedDimensions.length > 0 ? (
        <Txt className="mt-[9px] text-[11.5px] text-ink-faint" style={{ lineHeight: 17 }}>
    ...
    <View className="gap-4">
      <View className="flex-row items-center justify-between">
        <Mono className="text-[9px]" style={{ letterSpacing: 1.2, color: C.grn }}>
          SACI AUDIT · {report.rubricVersion}
        </Mono>
            ...
            <View key={`${finding.code}-${index}`} className="mt-[10px]">
              <View className="mb-[4px] flex-row items-center gap-2">
                <View
                  ...
                  style={{ backgroundColor: finding.severity === 'high' ? C.red : C.amb }}
                />
                <Mono className="text-[9.5px] text-ink-faint" style={{ letterSpacing: 0.8 }}>
              ...
              {finding.fields.length > 0 ? (
                <Mono className="mt-[3px] text-[10px] text-ink-faint">
                  {finding.fields.join(' · ')}
                </Mono>
              ) : null}
            </View>
          ...
          {[...priorities, ...rest].map((item, index) => (
            <View key={`${item.dimension}-${index}`} className="mt-[11px] flex-row gap-[9px]">
              ...
              <View className="flex-1">
                <Txt className="text-[12.5px] text-ink" style={{ lineHeight: 19 }}>
                  {item.action}
                </Txt>
                <Mono className="mt-[3px] text-[10px] text-ink-faint">
                  {item.dimension}
